...
class DB
{
    ...
    {
        $message = '<p>Quando aconteceu '. date('d/m/Y H:i:s').':</p>';
        $message .= '<p>Query: '. htmlentities( $query ).'<br />';
        $message .= 'Qual erro retornado: ' . $error;
        $message .= '</p>';
        ...
        if( defined( 'SEND_ERRORS_TO' ) )
        {
            ...
            $headers .= 'To: Elizeu <'.SEND_ERRORS_TO.'>' . "\r\n";
            $headers .= 'From: Central do Franqueado <atendimento@web4comunicacao.com>' . "\r\n";

            mail( SEND_ERRORS_TO, 'Importante: Erro em uma Query !!', $message, $headers );   
        }
        else
        {
